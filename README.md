# PS One

알고리즘과 SQL 문제 풀이를 기록하는 저장소입니다.

## 프로그래머스 알고리즘

| 번호 | 문제 | 난이도 | 분류 | 풀이 |
| --- | --- | --- | --- | --- |
| 43165 | [타겟 넘버](https://school.programmers.co.kr/learn/courses/30/lessons/43165?language=java) | Level 2 | DFS, 재귀, 백트래킹 | [풀이](./programmers/algorithm/dfs-bfs/43165-타겟-넘버/) |
| 43238 | [입국심사](https://school.programmers.co.kr/learn/courses/30/lessons/43238) | Level 3 | 이분 탐색, Parametric Search | [풀이](./programmers/algorithm/binary-search/43238-입국심사/) |
| 43236 | [징검다리](https://school.programmers.co.kr/learn/courses/30/lessons/43236) | Level 4 | 이분 탐색, Parametric Search, 그리디 | [풀이](./programmers/algorithm/binary-search/43236-징검다리/) |

## 프로그래머스 SQL

| 번호 | 문제 | 난이도 | 분류 | 풀이 |
| --- | --- | --- | --- | --- |
| 131534 | [상품을 구매한 회원 비율 구하기](https://school.programmers.co.kr/learn/courses/30/lessons/131534) | Level 5 | JOIN, GROUP BY, DISTINCT, 서브쿼리 | [풀이](./programmers/sql/join/131534-상품을-구매한-회원-비율-구하기/) |
| 133024 | [인기있는 아이스크림](https://school.programmers.co.kr/learn/courses/30/lessons/133024) | Level 1 | SELECT, ORDER BY | [풀이](./programmers/sql/select/133024-인기있는-아이스크림/) |
| 133027 | [주문량이 많은 아이스크림들 조회하기](https://school.programmers.co.kr/learn/courses/30/lessons/133027) | Level 4 | JOIN, GROUP BY | [풀이](./programmers/sql/join/133027-주문량이-많은-아이스크림들-조회하기/) |
| 276035 | [FrontEnd 개발자 찾기](https://school.programmers.co.kr/learn/courses/30/lessons/276035) | Level 4 | JOIN, 비트 연산 | [풀이](./programmers/sql/join/276035-frontend-개발자-찾기/) |

## PostgreSQL Exercises SQL

| 문제 | DBMS | 분류 | 풀이 |
| --- | --- | --- | --- |
| [다른 회원을 추천한 모든 회원 목록 조회하기](https://pgexercises.com/questions/joins/self.html) | PostgreSQL | SELF JOIN, DISTINCT, ORDER BY | [풀이](./postgresql-exercises/sql/joins/self-members-who-recommended-another-member/) |
